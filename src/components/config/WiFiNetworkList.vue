...
<template>
  <div class="rounded-2xl border border-gray-200/80 bg-white shadow-sm overflow-hidden">
    <!-- Header -->
    <div
      class="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-sky-50/80 to-indigo-50/50">
      ...
      <button @click="emit('scan')" :disabled="config.isScanning"
    ...
    <div v-else class="divide-y divide-gray-50 max-h-[420px] overflow-y-auto">
      <div v-for="network in config.sortedNetworks" :key="network.bssid" :class="[
        ...
        <div class="flex items-center gap-3.5">
          ...
          <div class="min-w-0">
            <div class="flex items-center gap-2">
              ...
              <!-- Connected badge -->
              <span v-if="config.espStatus.connected && config.espStatus.ssid === network.ssid"
        ...
        <!-- Select indicator -->
        <div v-if="config.selectedSSID === network.ssid"
          class="w-2 h-2 rounded-full bg-sky-500 shrink-0 animate-pulse" />
      </div>
    </div>

    <!-- Scanning state -->
    <div v-if="config.isScanning" class="px-6 py-4 bg-sky-50/50 border-t border-sky-100 flex items-center gap-3">
