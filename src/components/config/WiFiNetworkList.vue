...
const rssiLevels = [
  { min: -50, bars: 4, text: 'text-emerald-500', bar: 'bg-emerald-400' },
  { min: -60, bars: 3, text: 'text-sky-500', bar: 'bg-sky-400' },
  { min: -70, bars: 2, text: 'text-amber-500', bar: 'bg-amber-400' },
  { min: -Infinity, bars: 1, text: 'text-red-500', bar: 'bg-red-400' },
] as const

function rssiLevel(rssi: number) {
  return rssiLevels.find((l) => rssi >= l.min)!
}
  ...
  <div class="rounded-2xl border border-gray-200/80 bg-white shadow-sm overflow-hidden">
    ...
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-sky-50/80 to-indigo-50/50">
      ...
      <button
        ...
        class="flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-semibold transition-all duration-200 bg-sky-500 text-white hover:bg-sky-600 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed shadow-sm shadow-sky-200"
      >
    ...
    <!-- Empty state -->
    <div v-if="!config.isScanning && config.scannedNetworks.length === 0" class="px-6 py-12 text-center">
          ...
          <div class="flex items-end gap-0.5 h-5 w-5 shrink-0">
            <div
              ...
              :class="[
                ...
                i <= rssiLevel(network.rssi).bars
                  ? (config.espStatus.connected && config.espStatus.ssid === network.ssid
                      ? 'bg-emerald-500'
                      : rssiLevel(network.rssi).bar)
                  : 'bg-gray-200',
              ]"
          ...
          <div class="min-w-0">
            ...
            <div class="flex items-center gap-3 mt-0.5">
              <span class="text-[11px] text-gray-400">
                {{ config.encryptionLabel(network.encryption) }}
              </span>
              <span class="text-[11px]" :class="rssiLevel(network.rssi).text">
