...
<template>
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full min-h-0">
    <!-- Left Column: Configs -->
    <div class="flex flex-col gap-6 min-h-0 overflow-y-auto custom-scrollbar">
      <!-- STA Configuration -->
      <div class="rounded-2xl border border-gray-200/80 bg-white shadow-sm flex flex-col shrink-0">
        <div class="p-6 space-y-5">
          ...
          <div class="space-y-4">
            <div>
              <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-2 ml-1">SSID</label>
      ...
      <div class="rounded-2xl border border-gray-200/80 bg-white shadow-sm flex flex-col shrink-0">
        <div class="p-6 space-y-5">
          ...
          <div class="space-y-4">
            <div>
              <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-2 ml-1">Tên WiFi
                (SSID)</label>
    ...
    <div class="rounded-2xl border border-gray-200/80 bg-white shadow-sm flex flex-col h-full min-h-0 overflow-hidden">
      <!-- Header Fixed -->
      <div class="p-6 border-b border-gray-100 flex items-center justify-between shrink-0 bg-white z-10">
        <div>
          <h3 class="text-sm font-bold text-gray-800">WiFi Networks</h3>
          <p class="text-xs text-gray-400 mt-0.5">Danh sách mạng xung quanh</p>
        </div>
        <button
          class="flex items-center gap-2 rounded-xl bg-gray-800 px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-gray-700 active:bg-gray-900 transition-all font-medium">
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar w-full p-4 space-y-2.5 bg-gray-50/50">
        <div v-if="device.scanResults.length === 0"
          class="flex flex-col items-center justify-center h-full text-gray-400 space-y-3 py-10">
          <ScanSearch class="w-12 h-12 opacity-20" />
          <p class="text-sm font-medium">Chưa có dữ liệu scan.</p>
        </div>
